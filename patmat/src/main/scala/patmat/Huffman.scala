...
object Huffman {
  ...
  if (trees.length < 2) {
    ...
    val t = List(Fork(trees.head, trees(1), chars(trees.head) ::: chars(trees(1)), weight(trees.head) + weight(trees(1)))) ::: trees.takeRight(trees.length - 2)
    t.sortBy(weight(_))
  }
  ...
  /**
    ...
    */
  def until[T](c: List[T] => Boolean, f: List[T] => List[T])(xs: List[T]): T =
  if (c(xs)) xs.head else until(c, f)(f(xs))
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    def f(t: CodeTree, b: List[Bit]): (Char, List[Bit]) =
      t match {
        case Fork(l, r, _, _) =>
          b match {
            case 0 :: xs => f(l, xs)
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def f(t: CodeTree, c: Char): List[Bit] = t match {
        case Leaf(_, _) => Nil
        case Fork(l, r, _, _) =>
